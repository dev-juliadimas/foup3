...
class Eixo extends BDados
{
	public $bloco = array("tabela" => "eixo", "order_by" => "id");
	
	public $dados = 0;

	// url da lista
	public $url_lista = "eixos.php";
    ...
    public function GetMembros($id_eixo, $id_status)
    {
        $sql = "select * from eixo_membro where id_eixo = ".(int)$id_eixo." and id_status = '$id_status' order by nm_membro";
        $res = mysqli_query($this->con, $sql);
    ...
    public function GetMembroByEmail($email)
    {
        $sql = "select * from eixo_membro
                where email = '$email'";
        $res = mysqli_query($this->con, $sql);
